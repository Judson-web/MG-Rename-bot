...
async def progress_bar(current, total, status_msg, start, msg):
    ...
    if round((present - start) % 3) == 0 or current == total:
        ...
        progressbar = "[{0}{1}]".format(\
            ''.join([f"{small_orange_diamond}" for i in range(math.floor(percentage / 10))]),
            ''.join([f"{small_blue_diamond}" for i in range(10 - math.floor(percentage / 10))])
            )
